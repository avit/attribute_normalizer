...
By default, the normalization happens on both read and write operations. To change this, you can specify when the normalization should happen with the <code>:on</code> option:

<pre><code>
  normalize_attributes :home_phone_number,  :on => :write
  normalize_attributes :compacted_field,    :on => :read
</code></pre>
